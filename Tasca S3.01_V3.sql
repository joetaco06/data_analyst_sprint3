...
# *Actualizado: Definimos como primary key a id desde su línea de código. Modificamos el tipo de variable de los 
# campos PIN y CVV a CHAR(4) y CHAR(3), respectivamente.
...
# Averiguamos el nombre de la foreign key en transaction para eliminarla ya que luego
# necesitaremos crear las relaciones a través de las foreign keys.

SELECT constraint_name
FROM information_schema.key_column_usage
WHERE table_name = 'transaction'
AND constraint_schema = 'transactions';

ALTER table transaction
DROP foreign key transaction_ibfk_1;

# Creamos las Foreign Keys en transaction que referencian a las primary keys en las tablas credit_card y company:
...
/* *Actualización: Ya no se utilizará la vía de desactivar las Foreign Key para realizar modificaciones. En su lugar; y
para mantener la integridad referencial; ingresaremos los datos en las correspondientes tablas el siguiente
orden: credit_card y company. Luego de ello recién se procede a ingresar los datos de la tabla transaction. Para las 
tablas credit_card y company basta con ingresar sus respectivas Primary Keys; para la tabla transaction se ingresan
los datos de todos los campos a excepción de timestamp, pues no se proporciona y no es obligatorio, por lo que quedaría
reflejado como NULL.
...
INSERT INTO transaction (id, credit_card_id, company_id, user_id, lat, longitude, amount, declined) VALUES 
	('108B1D1D-5B23-A76C-55EF-C568E49A99DD', 'CcU-9999', 'b-9999', '9999', '829.999', '-117.999', '111.11', '0')
    ;

# Se verifica que se creó correctamente el nuevo registro:
...
# la base de datos.
/* *Actualización: Ya no se realiza la eliminación del registro a través de la desactivación de la clave foránea. 
  Ahora que hemos establecido correctamente la relación y cardinalidad entre las tablas de hechos y dimensiones
  la eliminación del registro en transaction se ejecuta directamente. */
...
# * Actualización: Se utilizará la sentencia ORDER BY de manera externa (es decir, al consultar la vista) y 
# no dentro de la creación de la vista, como se había realizado anteriormente.

CREATE VIEW VistaMarketing AS
SELECT company_name, phone, country, ROUND(AVG(amount), 2) AS compra_media
FROM company
...
# Ejercicio 3.1 : Observando el diagrama de relación de tablas podemos notar que hay una serie de
# comandos que se han ejecutado y los describiremos en los pasos siguientes.

#Paso 1: Se ha creado la tabla "user" siguiendo la instrucción del script "estructura_datos_user":
...
# Creamos la Foreign key de transactions que referencia la primary key en user:
...
#Paso 2: Se carga toda la data del script "datos_introducir_user"
...
credit_card.iban AS iban_tarjeta_credito, transaction.amount AS monto_transaccion, company.company_name AS nombre_compania
FROM transaction
